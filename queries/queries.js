...
// Función para obtener la fecha actual de la base de datos
const getDate = async () => {
    try {
...
const deleteUserQuery = async (id) => {
    try {
        ...
        // Si no hay transferencias asociadas, proceder con la eliminación del usuario
...
const addTransferQuery = async (datos) => {
    ...
    const { id: emisorId } = (await pool.query(`SELECT id FROM usuarios WHERE nombre = '${emisor}'`)).rows[0];
    const { id: receptorId } = (await pool.query(`SELECT id FROM usuarios WHERE nombre = '${receptor}'`)).rows[0];
